...
var modulesDir = _normalize(__dirname).split('/').slice(0, -4);

function _combine(_, path, rel) {
	var cut = path.lastIndexOf('/');
	if (cut <= 0) throw new Error("too many parent dirs" + rel);
	path = path.substring(0, cut);
	while (rel.indexOf('./.') == 0) // get rid of leading ./. and ./..
		rel = rel.substring(2);
	if (rel.indexOf('../') == 0) return _combine(_, path, rel.substring(1));
	if (rel.indexOf('./') != 0) {
		var d = rel.split('/')[0];
		for (var m = modulesDir.slice(0); m.length > 0; m.pop()) {
...
function _directDependencies(_, path) {
	...
	while (match = requireRE.exec(str)) {
		result.push(_combine(_, path, match[2]));
	}
	return result;
}
