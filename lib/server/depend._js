...
function _combine(_, root, path, rel) {
	...
	if (path) {
		...
		while (rel.indexOf('./.') == 0) // get rid of leading ./. and ./..
			rel = rel.substring(2);
		if (rel.indexOf('../') === 0) return _combine(_, root, path, rel.substring(1));
		if (rel.indexOf('./') === 0) return path + rel.substring(1);
	}
	var d = rel.split('/')[0];
	for (var m = root.split('/'); m.length > 0; m.pop()) {
		var pfx = m.join('/') + '/node_modules/';
		if (fs.exists(pfx + d, _)) return  pfx + rel;
	}
	throw new Error('cannot resolve module: ' + rel);
}

exports.resolve = _combine;
...
function _directDependencies(_, root, path) {
	...
	while (match = requireRE.exec(str)) {
		result.push(_combine(_, root, path, match[2]));
	}
	return result;
}
...
function _missingDependencies(_, root, path, known) {
	...
	function _explore(_, path, missingMap) {
		...
		knownMap[path] = true;
		var dependencies = _directDependencies(_, root, path);
...
exports.directDependencies = function(_, root, path) {
	return funnel(_, function(_) {
		return _directDependencies(_, root, path);
	});
};
