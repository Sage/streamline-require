...
function _readFile(_, dep) {
	...
	_watch(path);
	if (/\._(js|coffee)$/.test(path)) {
		// transform to get runtime dependencies (for ex. regenerator)
		return require("streamline/lib/transform").transformFile(_, path, exports.loadOptions).code;
	} else {
...
function resolveDir(_, dep, path) {
	...
	var extra = ['index.js', 'index._js', 'index.es5', 'main.js', 'main._js', 'main.es5'].filter_(_, function(_, extra) {
...
function resolvePath(_, dep) {
	...
	var shadowRoot = exports.loadOptions.shadowRoot;
	if (shadowRoot) {
		res = resolveFile(_, dep, fspath.join(shadowRoot, dep.requiredAs));
		if (res) return res;
	}
	throw new Error("invalid require path: " + requireChain(dep));
}
