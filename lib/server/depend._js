...
function resolvePath(_, dep) {
	...
	// try npm-cache as last resort
	var npmCachePrefix = exports.loadOptions.npmCachePrefix;
	if (npmCachePrefix) {
		res = resolveFile(_, dep, fspath.join(dep.root, npmCachePrefix, dep.requiredAs));
		if (res) return res;
	}
	throw new Error("invalid require path: " + requireChain(dep));
}
