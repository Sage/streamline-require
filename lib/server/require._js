...
exports.dispatcher = function(config) {
	...
	depend.loadOptions.shadowRoot = shadowRoot; // clean this hack later

	function under(path, ref) {
		return path.substring(0, ref.length) === ref;
	}

	function path2Location(path) {
		var location = path;
		...
		try {
			...
			if (path) {
				...
				doMultipart(_, depend.etag(), Object.keys(missing), function(_, path) {
					...
					var location = path2Location(path);
					return cache(_, depend.etag(), "part:" + location, function(_) {
						checkWhiteList(location);
						var contents = /\/deps\//.test(resolvedAs) || /\.es5$/.test(resolvedAs)
							? fs.readFile(resolvedAs, 'utf8', _) 
							: require("streamline/lib/transform").transformFile(_, resolvedAs, depend.loadOptions).code;
						return {
							location: location,
				...
				var missing = cache(_, depend.etag(), "localize#" + known, function(_) {
					...
					while (known.length > 0) {
						path = known.shift();
						var deps = depend.missingDependencies(_, root, path, processed);
						Object.keys(deps).forEach(function(path) {
							missing.push({
								path: path,
								resolvedAs: deps[path],
							});
						});
						processed.push(path);
					}
				...
				doMultipart(_, depend.etag(), missing, function(_, dep) {
					...
					var location = path2Location(dep.path);
					checkWhiteList(location);
					return {
						location: location,
