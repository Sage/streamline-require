...
exports.dispatcher = function(config) {
	...
	var npmCachePrefix = config.npmCachePrefix || "cached-modules/node_modules/";
	depend.loadOptions.npmCachePrefix = npmCachePrefix; // clean this hack later


	function uncache(location) {
		if (location.substring(0, npmCachePrefix.length) === npmCachePrefix) 
			location = location.substring(npmCachePrefix.length);
		return location;
	}
