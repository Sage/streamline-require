	...
	return function(_, request, response) {
		...
		try {
			...
			if (path) {
				...
				path = depend.resolve(_, root, null, path); //path[0] == '/' ? root + path : root + "/node_modules/" + path;
				var stats = fs.stat(path + (_exists(_, path + "._js") ? "._js" : ".js"), _);
				if (!stats.isFile()) return _replyError(response, 404, "file not found " + path);

				var missing = depend.missingDependencies(_, root, path, known);
				...
				while (known.length > 0) {
					...
					missing = missing.concat(depend.missingDependencies(_, root, path, processed));
